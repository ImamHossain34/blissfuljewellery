...
fetch('../images.json') // Replace with the actual path to your JSON file
    ...
    .then(jsonData => {
        ...
        jsonData.forEach(productww => {
            if (productww.id) {
                // Create a new div or directly append the id to the container
                // const productIdDiv = document.createElement('div');
                // productIdContainer.innerHTML = `${productww.id}`; // Append the ID
                // productIdContainer.appendChild(productIdDiv);
            }
            addCartToHTML(productww.id);
        });
    })
    .catch(error => console.error('Error fetching JSON:', error));


function addCartToHTML(iddd = null){
    // clear data default
    ...
    if (cartList) {
      cartList.forEach(product => {
          if (product && product.id === iddd) { // Check if product id is 101
              ...
              newCart.innerHTML = 
                  `
                    ...
                    <div class="removeBtn" style="display:none;" onclick="removeFromCart(${product.id})">poopp</div>
                  `;
...
function removeFromCart(removeCartProduct){
  ...
  addCartToShop(removeCartProduct);
}
